...
- Sale cash-flow column count follows the selected escrow rule: Progress Drawdown CP+24, Staged Escrow and 10/90 CP+12, No Escrow CP+6, Closed-Loop Escrow max(CP+24, last shifted sales month + 1), Project Guarantee Account CP + retention months (default CP+12, minimum 12). Location + asset class only pre-select a default (Australia → 10/90 all classes; Dubai → Staged all classes; Abu Dhabi → Project Guarantee Account all sale classes; Malaysia → Progress residential / No Escrow commercial; China residential landed or high-rise → Closed-Loop; all other locations → none). All six rules remain selectable. Closed-loop locks 100% of buyer funds until practical completion and retains 3% of building works until CP+24. The topping-out sales shift applies in any jurisdiction only while the closed-loop topping-out toggle is on (default on at 50% for China, off elsewhere). The 70% of TDC construction-loan cap and 100% land equity apply only while closed-loop is selected in China. Abu Dhabi suspends the land term loan only while the Project Guarantee Account rule is selected; switching rules restores the stored land split and never overwrites it. The guarantee-account lender sweep is mandatory whenever a construction loan is outstanding and never touches a land loan. Guarantee retention defaults to a fixed percent of C1 construction cost in Abu Dhabi and to a percent of cumulative escrow proceeds elsewhere; after completion, collections top that target up before any developer release. ADREC/DMT is a local-regime note for Abu Dhabi only. Unset mode must not silently inherit staged/UAE escrow logic.
