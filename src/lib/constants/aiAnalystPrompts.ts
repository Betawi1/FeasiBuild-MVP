...
- Sale cash-flow column count follows the selected escrow rule: Progress Drawdown CP+24, Staged Escrow and 10/90 CP+12, No Escrow CP+6, Closed-Loop Escrow max(CP+24, last shifted sales month + 1). Location + asset class only pre-select a default (Australia → 10/90 all classes; Dubai/UAE → Staged all classes; Malaysia → Progress residential / No Escrow commercial; China residential landed or high-rise → Closed-Loop; all other locations → none). All five rules remain selectable. Closed-loop locks 100% of buyer funds until practical completion and retains 3% of building works until CP+24. The topping-out sales shift applies in any jurisdiction only while the closed-loop topping-out toggle is on (default on at 50% for China, off elsewhere). The 70% of TDC construction-loan cap and 100% land equity apply only while that rule is selected in China; switching rules restores standard timing, loan cap, and the stored land split. Unset mode must not silently inherit staged/UAE escrow logic.
