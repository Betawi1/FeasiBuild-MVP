...
export const SALE_CASH_OUTFLOW_AUDIT_FIELDS: Record<string, SaleAuditFieldMeta> = {
  ...
  basements: { label: "Basements (No. of levels)", uiStep: 4, type: "input" },
  podiumFloors: { label: "Podium / Parking Floors", uiStep: 4, type: "input" },
  towerFloors: { label: "Tower Floors", uiStep: 4, type: "input" },
  landedUnits: { label: "Number of Units", uiStep: 4, type: "input" },
  landedLandAreaPerUnit: {
    label: "Land Area per Unit (sqft)",
    uiStep: 4,
    type: "input",
  },
  landedBUAPerUnit: { label: "BUA per Unit (sqft)", uiStep: 4, type: "input" },
  hasRetailComponent: {
