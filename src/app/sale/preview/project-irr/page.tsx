...
export default function PreviewProjectIRRPage() {
  ...
  useEffect(() => {
    ...
    console.log("🔗 [Sale IRR Preview Wired]:", {
      stream: "sale",
      hasProjectInfo: !!projectInfo,
      hasCashOutflows: !!cashOutflows,
      hasCashInflows: !!cashInflows,
      constructionPeriod,
      totalMonths,
      tdc: cashOutflows.tdc,
      netProceeds: cashInflows.netProceeds,
    });
  ...
  useEffect(() => {
    // eslint-disable-next-line no-console
    console.log("🔍 ========== NET CASH FLOW SYNC CHECK ==========");
    // eslint-disable-next-line no-console
    console.log("🔍 Project IRR M0-M5:", cashFlows.slice(0, 6).map((cf) => cf.amount));
    // eslint-disable-next-line no-console
    console.log("🔍 Should match Cash Inflows preview exactly");
    // eslint-disable-next-line no-console
    console.log("================================================");
  }, [cashFlows]);
  ...
  const exportRows = useMemo(() => {
    ...
    const roundWhole = (v: number) => Math.round(v); // Full figures, no scaling

    const ncfRow: (string | number | null)[] = [
      "Net Cash Flow",
      ...months.map((_, idx) => {
        const k = roundWhole(netCashFlows[idx] || 0);
        return k === 0 ? null : k;
      }),
      roundWhole(netCashFlowTotal) === 0 ? null : roundWhole(netCashFlowTotal),
    ];
    ...
    const dcfRow: (string | number | null)[] = [
      ...
      ...months.map((_, idx) => {
        const k = roundWhole(discountData.discountedCashFlows[idx] || 0);
        return k === 0 ? null : k;
      }),
      roundWhole(discountedCashFlowTotal) === 0
        ? null
        : roundWhole(discountedCashFlowTotal),
    ];

    const npvRow: (string | number | null)[] = [
      "Cumulative NPV",
      ...months.map((_, idx) => {
        const k = roundWhole(discountData.cumulativeNPV[idx] || 0);
        return k === 0 ? null : k;
      }),
      roundWhole(cumulativeNPVFinal) === 0 ? null : roundWhole(cumulativeNPVFinal),
    ];
    ...
    const summaryRows: (string | number | null)[][] = [
      ...
      [
        ...
        roundWhole(unleveredMetrics.capitalInvested),
      ],
      [
        "Total returns (net proceeds) ('000)",
        roundWhole(unleveredMetrics.totalReturns),
      ],
      [
      ...
      ["Σ discounted cash flows NPV ('000)", roundWhole(discountedSum)],
    ];
  ...
  const formatNumber = (value: number) => {
    ...
    return new Intl.NumberFormat("en-US", { maximumFractionDigits: 0 }).format(
      Math.round(value) // Removed / 1000 - show full figures
    );
  };
  const formatThousandsOneDecimal = (value: number) =>
    ...
    }).format(value); // Removed / 1000 - show full figures
      ...
      <div className="mx-auto max-w-7xl">
        <div className="mb-6">
          ...
          <p className="text-sm text-slate-400">
            Unlevered IRR from Component 2 inflows and Component 1 outflows. Values shown in {currency}.
          </p>
        </div>
        ...
        <div className="bg-slate-800/50 border border-slate-700 rounded-lg p-6 mb-6 overflow-x-auto">
          ...
          <p className="mt-4 text-xs text-slate-500">
            ...
            Final NPV: {formatNumber(finalCumulativeNPV)}.
          </p>
        </div>
