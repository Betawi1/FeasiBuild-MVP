...
/** Pre-calculated monthly row from the financing engine (Malaysia / HDA progress payment model). */
...
export type CashFlowTableMalaysiaProps = {
  data: MonthlyRow[];
...
function TableRow({
  ...
  isNCF?: boolean;
  isPercent?: boolean;
  /** End-of-period balance row (TOTAL = last month, not sum of months). */
  isStockSeries?: boolean;
}) {
  const rowSum = data.reduce((sum, row) => sum + getValue(row), 0);
  const sumZero = rowSum === 0 || Object.is(rowSum, -0);
  const lastVal = data.length > 0 ? getValue(data[data.length - 1]) : 0;
    ...
    <tr className="border-b border-slate-800 transition-colors hover:bg-slate-800/30">
      ...
      {data.map((row) => {
        ...
        const isZero =
          !isStockSeries &&
          !isNCF &&
          (val === 0 || (typeof val === "number" && Object.is(val, -0)));
