...
function CashInflowsPageContent() {
  ...
  const cashInflows = useSaleModelStore((s) => s.cashInflows);
  ...
  useEffect(() => {
    console.log("💰 [Sale C2] Cash Inflows Mounted");
  }, []);

  useEffect(() => {
    console.log("🔍 [Sale C2] Store Isolation Check:", {
      hasSaleStore: typeof useSaleModelStore !== "undefined",
      cashInflowsLoaded: !!cashInflows,
      streamPrefix: STREAM,
    });
  ...
  useEffect(() => {
    ...
    if (uiStep === 2 && !cashInflowStepVisitLogged.current.has(2)) {
      cashInflowStepVisitLogged.current.add(2);
      logSaleCashInflow("salesPrice", cashInflows.salesPrice, 2);
    }

    if (uiStep === 3 && !cashInflowStepVisitLogged.current.has(3)) {
      cashInflowStepVisitLogged.current.add(3);
      logSaleCashInflow(
    ...
    if (uiStep === 4 && !cashInflowStepVisitLogged.current.has(4)) {
      cashInflowStepVisitLogged.current.add(4);
      logSaleCashInflow("buyerDownPayment", cashInflows.buyerDownPayment ?? 0, 4);
      logSaleCashInflow(
        "downPaymentMonths",
        cashInflows.downPaymentMonths?.length ?? 3,
        4
      );
      logSaleCashInflow(
    ...
    if (uiStep === 5 && !cashInflowStepVisitLogged.current.has(5)) {
      cashInflowStepVisitLogged.current.add(5);
      logSaleCashInflow("salesUptakeMode", cashInflows.salesUptake.mode, 5);
      if (cashInflows.salesUptake.mode === "preset") {
        logSaleCashInflow("salesUptakePreset", cashInflows.salesUptake.preset, 5);
      }
    }

    if (uiStep === 6 && !cashInflowStepVisitLogged.current.has(6)) {
      cashInflowStepVisitLogged.current.add(6);
      logSaleCashInflow("cashBuyerPercent", cashInflows.buyerMix.cashBuyerPercent, 6);
      logSaleCashInflow(
      ...
      logSaleCashInflow("vatPercent", cashInflows.buyerMix.vatPercent, 6);
      logSaleCashInflow("escrowFeePercent", cashInflows.buyerMix.escrowFeePercent, 6);
      logSaleCashInflow(
    ...
    if (uiStep === 7 && !cashInflowStepVisitLogged.current.has(7)) {
      cashInflowStepVisitLogged.current.add(7);
      logSaleCashInflow("defaultRate", cashInflows.defaultRate, 7);
      logSaleCashInflow(
    ...
    if (uiStep === 8 && !cashInflowStepVisitLogged.current.has(8)) {
      cashInflowStepVisitLogged.current.add(8);
      logSaleCashInflow(
        "launchMonthOffset",
        cashInflows.launchTiming.launchMonthOffset,
        8
      );
      logSaleCashInflow(
  ...
  const auditCashInflowChange = (
    field: string,
    value: string | number | boolean
  ) => {
    if (field in SALE_CASH_INFLOW_AUDIT_FIELDS) {
      logSaleCashInflow(field, value);
    }
  };
  ...
  const updateFormData = (field: string, value: unknown) => {
    ...
    if (field === "buyerDownPayment") {
      const n = Number(value) || 0;
      updateCashInflows({ buyerDownPayment: n });
      auditCashInflowChange("buyerDownPayment", n);
      return;
    }

    if (field === "downPaymentMonths") {
      const count = Math.max(1, Math.round(Number(value) || 1));
      updateCashInflows({
        downPaymentMonths: Array.from({ length: count }, (_, i) => i),
      });
      auditCashInflowChange("downPaymentMonths", count);
      return;
    }

    if (field === "saleableBUARatio" || field === "salesPrice" || field === "defaultRate") {
      const v = value as number;
      updateCashInflows({ [field]: v } as Partial<CashInflows>);
      auditCashInflowChange(field, v);
      return;
    }
    ...
    if (salesUptakeFields.has(field)) {
      ...
      if (field === "salesUptakeMode" || field === "salesUptakePreset") {
        auditCashInflowChange(field, value as string);
      }
      return;
    }
  ...
  // Helper to get pretty building subtype name for profile bar
