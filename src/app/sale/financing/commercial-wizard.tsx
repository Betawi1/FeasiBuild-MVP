...
export type JurisdictionId = "UAE" | "KSA" | "Malaysia" | "Australia" | "Vietnam" | "Thailand";
...
function resolveJurisdiction(projectInfo: ProjectInfo): JurisdictionId {
  ...
  if (code === "TH" || c.includes("thailand")) return "Thailand";
  return "UAE";
}
...
function CommercialFinancingWizardContent() {
  ...
  const goToStep = useCallback(
    (stepIndex: number) => {
      ...
      router.push(
        withStreamPrefix(
          streamPrefix,
          `/financing?wizard=commercial&step=${clamped + 1}`
        )
      );
  ...
  useEffect(() => {
    ...
    if (currentStep === 4 && !financingStepVisitLogged.current.has(6)) {
    ...
    if (currentStep === 5 && !financingStepVisitLogged.current.has(7)) {
  ...
  const prefSharesAmount = formData.prefSharesEnabled
    ...
    : 0;
  const isLandEquityLocked = false; // Commercial: land term loan available in all jurisdictions
  ...
  const handleNext = useCallback(() => {
    ...
    if (currentStep >= maxStepIndex) {
      calculateFinancing();
      router.push(
        withStreamPrefix(streamPrefix, "/preview/financing?wizard=commercial")
      );
      return;
    }
    goToStep(currentStep + 1);
      ...
      <header className="sticky top-0 z-40 border-b border-slate-700 bg-slate-900/95 backdrop-blur">
        <div className="mx-auto max-w-6xl px-4 py-4">
          <h1 className="text-2xl font-bold text-white">
            Component 4: Commercial Financing (Sale)
          </h1>
          <p className="mt-1 text-sm text-slate-400">
            Land term loan + construction RCF • Sales recycling waterfall (no escrow)
          </p>
      ...
      <main className="mx-auto max-w-6xl px-4 py-12">
        ...
        {currentStep === 3 && (
          <Step4PreferenceShares
            prefSharesEnabled={formData.prefSharesEnabled ?? false}
            prefSharesAllocationPercent={formData.prefSharesAllocationPercent ?? 0}
            prefSharesReturnPct={formData.prefSharesReturnPct ?? 0}
            prefSharesReturnType={formData.prefSharesReturnType ?? "fixed-dividend"}
            cashEquityRequired={cashEquityRequired}
            ...
            onAllocationChange={(percent) =>
              updateField("prefSharesAllocationPercent", percent)
            }
            onReturnPctChange={(percent) => updateField("prefSharesReturnPct", percent)}
            onReturnTypeChange={(type) => updateField("prefSharesReturnType", type)}
          />
        )}
        ...
        {/* Step 7 — Sales recycling */}
        {currentStep === 6 && (
          <div className="space-y-6">
            <h2 className="text-xl font-semibold text-white">Sales Recycling</h2>
            <div className="space-y-4 rounded-lg bg-slate-800/80 p-6 ring-1 ring-slate-700">
              <p className="text-sm text-slate-300">
                Sales proceeds sweep directly to debt service and equity distribution.
                No escrow or trust accounts — standard commercial waterfall in the final step.
              </p>
              <label
...
export default function CommercialFinancingWizard() {
  return (
    <SearchParamsBoundary>
      <CommercialFinancingWizardContent />
    </SearchParamsBoundary>
  );
}
