...
export default function SaleFeasibilityStudyPage() {
  ...
  const generateReport = useCallback(async (options?: { force?: boolean }) => {
    ...
    try {
      ...
      try {
        ...
        const res = await fetch("/api/feasibility/generate-sale", {
          method: "POST",
  ...
  const handleBack = () => {
    router.push("/sale/preview/scenario-analysis");
  };
    ...
    return (
      <div className="min-h-screen bg-slate-900 flex flex-col items-center justify-center p-10">
        <p className="text-white text-lg animate-pulse">
          Generating sale-stream feasibility study…
        </p>
    ...
    <div className="min-h-screen bg-slate-950">
      <div className="no-print sticky top-0 z-50 border-b border-slate-800 bg-slate-900/90 px-6 py-3 backdrop-blur">
        <div className="mx-auto max-w-[1280px]">
          <h1 className="text-2xl font-bold text-white">{studyTitle}</h1>
