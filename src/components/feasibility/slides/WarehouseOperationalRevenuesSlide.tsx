...
interface Props extends SlideEditingProps {
  ...
  paragraphs?: string[];
  /** Label for the area metric under commentary (default: Warehouse BUA). */
  areaLabel?: string;
}

export default function WarehouseOperationalRevenuesSlide({
  data,
  paragraphs = [],
  isEditing = false,
  onParagraphChange,
  areaLabel = "Warehouse BUA",
}: Props) {
      ...
      <div className="flex-1 grid grid-cols-2 gap-6 min-h-0 overflow-hidden">
        <div className="space-y-2 overflow-y-auto">
          ...
          <p className="text-xs text-slate-500">
            {areaLabel}: {data.officeGla.toLocaleString()} sqft
          </p>
        </div>
